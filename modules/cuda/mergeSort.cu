...
template<uint sortDir> static inline __device__ uint binarySearchInclusive(float val, float *data, uint L, uint stride)
{
    if (L == 0)
    {
        return 0;
...
template<uint sortDir> static inline __device__ uint binarySearchExclusive(float val, float *data, uint L, uint stride)
{
    if (L == 0)
    {
        return 0;
...
)
{
    ...
    for (uint stride = 1; stride < arrayLength; stride <<= 1)
    {
        ...
        uint posA = binarySearchInclusive<sortDir>(keyA, baseKey + stride, stride, stride) + lPos;
        uint posB = binarySearchExclusive<sortDir>(keyB, baseKey +      0, stride, stride) + lPos;
...
)
{
    ...
    if (i < segmentSamplesA)
    {
        ...
        d_RanksB[i] = binarySearchInclusive<sortDir>(
    ...
    if (i < segmentSamplesB)
    {
        ...
        d_RanksA[(stride / SAMPLE_STRIDE) + i] = binarySearchExclusive<sortDir>(
...
)
{
    ...
    if (i < segmentSamplesA)
    {
        uint dstPos = binarySearchInclusive<1U>(d_Ranks[i], d_Ranks + segmentSamplesA, segmentSamplesB, nextPowerOfTwo(segmentSamplesB)) + i;
        d_Limits[dstPos] = d_Ranks[i];
    }

    if (i < segmentSamplesB)
    {
        uint dstPos = binarySearchExclusive<1U>(d_Ranks[segmentSamplesA + i], d_Ranks, segmentSamplesA, nextPowerOfTwo(segmentSamplesA)) + i;
...
)
{
    ...
    if (threadIdx.x < lenA)
    {
        ...
        dstPosA = binarySearchInclusive<sortDir>(keyA, srcBKey, lenB, nPowTwoLenB) + threadIdx.x;
    }
    ...
    if (threadIdx.x < lenB)
    {
        ...
        dstPosB = binarySearchExclusive<sortDir>(keyB, srcAKey, lenA, nPowTwoLenA) + threadIdx.x;
    }
...
{
    __shared__ float s_key[2 * SAMPLE_STRIDE];
...
)
{
    ...
    {//printf("stride: %d\n", stride);
        ...
        if (lastSegmentElements <= stride)
        {
			...
			//printf("N: %d, lastSegmentElements; %d\n", N, lastSegmentElements);
            checkCudaErrors(cudaMemcpy(okey + (N - lastSegmentElements), ikey + (N - lastSegmentElements), lastSegmentElements * sizeof(float), cudaMemcpyDeviceToDevice));
